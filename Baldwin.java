...
public class Baldwin extends Character implements Movable, Attackable, Rewardable, MeleeUnit {
    ...
    private static final int ATTACK_HIT_FRAME = 1;
    private static final int MAX_HEALTH = 110;
    private static final int ATTACK_DAMAGE = 20;
    private static final double ATTACK_SPEED_SECONDS = 1.2;
    ...
    public Baldwin(boolean facingRight) {
        super(MAX_HEALTH, ATTACK_DAMAGE, ATTACK_RANGE, MOVE_SPEED, facingRight, Faction.LIGHT);
        try {
            GreenfootSound spawnSound = new GreenfootSound("BaldwinSpawn.wav");
            spawnSound.setVolume(75);
            spawnSound.play();
        } catch (Exception e) {
            System.err.println("Peringatan: Gagal memuat suara spawn untuk " + this.getClass().getName());
        }
        idleSprites = SpriteLoader.loadSprites("light/Baldwin_Idle.png", 7);
        runSprites = SpriteLoader.loadSprites("light/Baldwin_Walk.png", 7);
        dieSprites = SpriteLoader.loadSprites("light/Baldwin_Die.png", 4);
        attackSprites = SpriteLoader.loadSprites("light/Baldwin_Attack.png", 4);
        if (!this.facingRight) {
            flipAllSprites();
        }
    ...
    public void die() {
        try {
            GreenfootSound dieSound = new GreenfootSound("Baldwindie.wav");
    ...
    private void setupAttack() {
        ...
        if (currentAttackSprites != null && currentAttackHitFrame >= currentAttackSprites.length) {
             System.out.println("!!! WARNING BALDWIN: HitFrame invalid !!!");
        ...
        switch (currentState) {
            ...
            case ATTACKING:
                if (attackFrame == currentAttackHitFrame && !attackHasHit) {
                    slash(null);
                    try {
                        GreenfootSound attackSound = new GreenfootSound("Baldwin-Attack.wav");
    ...
    public void slash(Character target) {
        if (getWorld() == null) return;
        int hitboxOffsetX = (facingRight ? 35 : -35);
