...
public class YunZhao extends Character implements Movable, Attackable, Rewardable, MeleeUnit {
    ...
    private static final double ATTACK_SPEED_SECONDS = 1.5;
    ...
    public YunZhao(boolean facingRight) {
        ...
        idleSprites = SpriteLoader.loadSprites("light/Yun Zhao_Idle.png", 4);
        runSprites = SpriteLoader.loadSprites("light/Yun Zhao_Walk.png", 8);
        dieSprites = SpriteLoader.loadSprites("light/Yun Zhao_Die.png", 6);
        attackSprites = SpriteLoader.loadSprites("light/Yun Zhao_Attack.png", 5);
        if (!this.facingRight) {
            flipAllSprites();
        }
    ...
    public void die() {
        try {
            GreenfootSound dieSound = new GreenfootSound("Yunzhaodie.wav");
    ...
    private void setupAttack() {
        ...
        if (currentAttackSprites != null && currentAttackHitFrame >= currentAttackSprites.length) {
             System.out.println("!!! WARNING YUNZHAO: HitFrame invalid !!!");
        ...
        switch (currentState) {
            ...
            case ATTACKING:
                if (attackFrame == currentAttackHitFrame && !attackHasHit) {
                    slash(null);
                    try {
                        GreenfootSound attackSound = new GreenfootSound("YunZhaoAttack.wav");
