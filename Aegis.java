...
public class Aegis extends Character implements Movable, Attackable, Rewardable, MeleeUnit {
    ...
    private static final int ATTACK_HIT_FRAME = 2;
    ...
    public Aegis(boolean facingRight) {
        super(MAX_HEALTH, ATTACK_DAMAGE, ATTACK_RANGE, MOVE_SPEED, facingRight, Faction.LIGHT);
        try {
            GreenfootSound spawnSound = new GreenfootSound("AegisSpawn.wav");
            spawnSound.setVolume(75);
            spawnSound.play();
        } catch (Exception e) {
            System.err.println("Peringatan: Gagal memuat suara spawn untuk " + this.getClass().getName());
        }
        idleSprites = SpriteLoader.loadSprites("light/Aegis_Idle.png", 7);
        runSprites = SpriteLoader.loadSprites("light/Aegis_Walk.png", 7);
        dieSprites = SpriteLoader.loadSprites("light/Aegis_Dead.png", 5);
        attackSprites = SpriteLoader.loadSprites("light/Aegis_Attack.png", 4);
        if (!this.facingRight) {
            flipAllSprites();
        }
    ...
    public void die() {
        try {
            GreenfootSound dieSound = new GreenfootSound("Aegisdie.wav");
    ...
    private void setupAttack() {
        ...
        if (currentAttackSprites != null && currentAttackHitFrame >= currentAttackSprites.length) {
             System.out.println("!!! WARNING AEGis: HitFrame invalid !!!");
        ...
        switch (currentState) {
            ...
            case ATTACKING:
                if (attackFrame == currentAttackHitFrame && !attackHasHit) {
                    slash(null);
                    try {
                        GreenfootSound attackSound = new GreenfootSound("AegisAttack.wav");
    ...
    public void slash(Character target) {
        if (getWorld() == null) return;
        int hitboxOffsetX = (facingRight ? 45 : -45);
