...
public class Hayato extends Character implements Movable, Attackable, Rewardable, MeleeUnit {
    ...
    private int runFrame = 0, runFrameDelay = 5, runFrameCounter = 0;
    private int dieFrame = 0, dieFrameDelay = 5, dieFrameCounter = 0;
    ...
    private static final int ATTACK_1_FRAME_DELAY = 4, ATTACK_1_HIT_FRAME = 2;
    private static final int ATTACK_2_FRAME_DELAY = 6, ATTACK_2_HIT_FRAME = 3;
    private static final int MAX_HEALTH = 120, ATTACK_DAMAGE = 25;
    private static final double ATTACK_SPEED_SECONDS = 1.0;
    private static final int ATTACK_RANGE = 5, MOVE_SPEED = 2, REWARD_VALUE = 20;
    ...
    public Hayato(boolean facingRight) {
        ...
        idleSprites = SpriteLoader.loadSprites("light/hayato_idle.png", 6);
        runSprites = SpriteLoader.loadSprites("light/hayato_walk.png", 8);
        dieSprites = SpriteLoader.loadSprites("light/hayato_die.png", 6);
        attack1Sprites = SpriteLoader.loadSprites("light/hayato_attack1.png", 4);
        attack2Sprites = SpriteLoader.loadSprites("light/hayato_attack2.png", 4);
        if (!this.facingRight) {
            flipAllSprites();
        }
    ...
    private void flipAllSprites() {
        ...
        for (GreenfootImage img : attack1Sprites) { img.mirrorHorizontally(); }
        for (GreenfootImage img : attack2Sprites) { img.mirrorHorizontally(); }
    }
    ...
    private void setupAttack() {
        ...
        if (currentAttackSprites != null && currentAttackHitFrame >= currentAttackSprites.length) {
             System.out.println("!!! WARNING HAYATO: HitFrame (" + currentAttackHitFrame + ") invalid for attack (" + currentAttackSprites.length + " frames) !!!");
        ...
        switch (currentState) {
            ...
            case ATTACKING:
                if (attackFrame == currentAttackHitFrame && !attackHasHit) {
                    slash(null);
                    try {
                        GreenfootSound attackSound = new GreenfootSound("Hayato-Attack.wav"); // Consider Hayato_attack1/2.wav?
