...
public class BarbarianKing extends Character implements Movable, Attackable, Rewardable, MeleeUnit {
    ...
    private static final double ATTACK_SPEED_SECONDS = 2.2;
    private static final int ATTACK_RANGE = 5;
    private static final int MOVE_SPEED = 1;
    private static final int REWARD_VALUE = 50;
    private int attackCooldown = (int)(ATTACK_SPEED_SECONDS * 60);
    private int currentCooldown = 0;

    public BarbarianKing(boolean facingRight) {
        super(MAX_HEALTH, ATTACK_DAMAGE, ATTACK_RANGE, MOVE_SPEED, facingRight, Faction.LIGHT);
        try {
            GreenfootSound spawnSound = new GreenfootSound("BarbarianKingSpawn.wav");
            spawnSound.setVolume(75);
            spawnSound.play();
        } catch (Exception e) {
            System.err.println("Peringatan: Gagal memuat suara spawn untuk " + this.getClass().getName());
        }
        idleSprites = SpriteLoader.loadSprites("light/BarbarianKing_Idle.png", 10);
        runSprites = SpriteLoader.loadSprites("light/BarbarianKing_Walk.png", 12);
        dieSprites = SpriteLoader.loadSprites("light/BarbarianKing_Die.png", 5);
        attackSprites = SpriteLoader.loadSprites("light/BarbarianKing_Attack.png", 5);
        if (!this.facingRight) {
            flipAllSprites();
        }
    ...
    public void die() {
        try {
            GreenfootSound dieSound = new GreenfootSound("BarbarianKingdie.wav");
    ...
    private void setupAttack() {
        ...
        if (currentAttackSprites != null && currentAttackHitFrame >= currentAttackSprites.length) {
             System.out.println("!!! WARNING BARBARIANKING: HitFrame invalid !!!");
        ...
        switch (currentState) {
            ...
            case ATTACKING:
                if (attackFrame == currentAttackHitFrame && !attackHasHit) {
                    slash(null);
                     try {
                        GreenfootSound attackSound = new GreenfootSound("BarbarianKingAttack.wav");
    ...
    public void slash(Character target) {
        if (getWorld() == null) return;
        int hitboxOffsetX = (facingRight ? 40 : -40);
