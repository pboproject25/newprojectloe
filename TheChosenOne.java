...
public class TheChosenOne extends Character implements Movable, Attackable, Rewardable, MeleeUnit {
    ...
    private static final int ATTACK_1_FRAME_DELAY = 6, ATTACK_1_HIT_FRAME = 3;
    ...
    private static final int REWARD_VALUE = 22;
    private int attackCooldown = (int)(ATTACK_SPEED_SECONDS * 60);
    private int currentCooldown = 0;

    public TheChosenOne(boolean facingRight) {
        super(MAX_HEALTH, ATTACK_DAMAGE, ATTACK_RANGE, MOVE_SPEED, facingRight, Faction.LIGHT);
        try {
            GreenfootSound spawnSound = new GreenfootSound("TheChosenOneSpawn.wav");
            spawnSound.setVolume(75);
            spawnSound.play();
        } catch (Exception e) {
            System.err.println("Peringatan: Gagal memuat suara spawn untuk " + this.getClass().getName());
        }
        idleSprites = SpriteLoader.loadSprites("light/chosen_idle.png", 7);
        runSprites = SpriteLoader.loadSprites("light/chosen_run.png", 8);
        dieSprites = SpriteLoader.loadSprites("light/chosen_dead.png", 5);
        attack1Sprites = SpriteLoader.loadSprites("light/chosen_attack1.png", 5);
        attack2Sprites = SpriteLoader.loadSprites("light/chosen_attack2.png", 5);
        if (!this.facingRight) {
            flipAllSprites();
        }
    ...
    public void die() {
        try {
            GreenfootSound dieSound = new GreenfootSound("TheChosenOnedie.wav");
    ...
    private void setupAttack() {
        ...
        if (currentAttackSprites!=null && currentAttackHitFrame >= currentAttackSprites.length) {
             System.out.println("!!! WARNING TheChosenOne: HitFrame invalid !!!");
        ...
        switch (currentState) {
            ...
            case ATTACKING:
                if (attackFrame == currentAttackHitFrame && !attackHasHit) {
                    slash(null);
                    try {
                        String attackSoundFile = (currentAttackSprites == attack1Sprites) ? "TheChosenOneAttack.wav" : "TheChosenOneAttack.wav";
