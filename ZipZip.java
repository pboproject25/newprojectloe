...
public class ZipZip extends Character implements Movable, Attackable, Rewardable, RangedUnit {
    ...
    private static final int ATTACK_HIT_FRAME = 10;
    ...
    public ZipZip(boolean facingRight) {
        ...
        try {
            GreenfootSound spawnSound = new GreenfootSound("ZipZip_spawn.wav");
            spawnSound.setVolume(75);
            spawnSound.play();
        } catch (Exception e) {
            System.err.println("Peringatan: Gagal memuat suara spawn untuk " + this.getClass().getName());
        }
        idleSprites = SpriteLoader.loadSprites("dark/zipzip_idle.png", 7);
        runSprites = SpriteLoader.loadSprites("dark/zipzip_walk.png", 7);
        dieSprites = SpriteLoader.loadSprites("dark/zipzip_dead.png", 5);
        attackSprites = SpriteLoader.loadSprites("dark/zipzip_light_charge.png", 13);
        if (!this.facingRight) {
            flipAllSprites();
        }
    ...
    public void die() {
        try {
            GreenfootSound dieSound = new GreenfootSound("ZipZip_die.wav");
        ...
        switch (currentState) {
            ...
            case ATTACKING:
                if (attackFrame == currentAttackHitFrame && !attackHasHit) {
                    ...
                    try {
                        GreenfootSound attackSound = new GreenfootSound("ZipZip_attack.wav");
